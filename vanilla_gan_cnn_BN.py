...
def generator(is_training=True):
    with tf.variable_scope("generator"):
        # z = [batch,62]
        # G_dense1 = slim.fully_connected(Z, 1024, activation_fn=tf.nn.relu, weights_initializer=w_init,normalizer_fn=slim.batch_norm)
        # G_dense1 = [batch,1024]
        G_dense2 = slim.fully_connected(Z, 128 * 7 * 7, activation_fn=tf.nn.relu, weights_initializer=w_init, normalizer_fn=slim.batch_norm)
        ...
        G_deconv2 = slim.conv2d_transpose(G_deconv1, 1, [4, 4], 2, activation_fn=tf.nn.sigmoid, weights_initializer=w_init, biases_initializer=b_init)
...
def discriminator(input, reuse=False, is_training=True):
    with tf.variable_scope("discriminator", reuse=reuse):
        ...
        D_conv2 = slim.conv2d(D_conv1, 128, [4, 4], 2, padding="SAME", activation_fn=tf.nn.relu,
                              weights_initializer=w_init, normalizer_fn=slim.batch_norm, reuse=reuse, scope="d_conv2")
        ...
        # D_dense1 = slim.fully_connected(D_conv2_flat, 1024, activation_fn=tf.nn.relu, weights_initializer=w_init, normalizer_fn=slim.batch_norm, reuse = reuse, scope = "d_dense1")
