...
def generator(is_training=True):
    with tf.variable_scope("generator"):
        ...
        # G_dense1 = [batch,1024]
        G_dense2 = slim.fully_connected(Z, 1024 * 30 * 40, activation_fn=tf.nn.relu, weights_initializer=w_init, normalizer_fn=slim.batch_norm)
        # G_dense1 = [batch,1024 * 30 * 30]
        G_dense2_flatt = tf.reshape(G_dense2, [-1, 30, 40, 1024])
